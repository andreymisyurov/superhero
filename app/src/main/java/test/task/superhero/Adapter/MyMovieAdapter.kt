package test.task.superhero.Adapter
...
class MyMovieAdapter(private val context: Context, private val movieList: MutableList<Movie>) :
    ...
    class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(listItem: Movie) {
            image.setOnClickListener {
                Toast.makeText(it.context, "нажал на ${txt_name.text}", Toast.LENGTH_SHORT)
                    .show()
            }
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        Picasso.with(context).load(movieList[position].imageurl).into(holder.image)
